...
#   $ModeratedBy=$_.ModeratedBy -split ';'
    ...
    if ($RecipientTypeDetails -eq "MailUniversalSecurityGroup")
        ...
        if ($ManagedBy)
            {
            New-DistributionGroup -Type security -Name $Name -Alias $Alias -DisplayName $DisplayName -PrimarySmtpAddress $smtp -MemberJoinRestriction $join -MemberDepartRestriction $depart -ManagedBy $ManagedBy
            Start-Sleep -s 10
            Set-DistributionGroup -Identity $alias -RequireSenderAuthenticationEnabled $RequireSenderAuthenticationEnabled
            }
            Else
            {
            New-DistributionGroup -Type security -Name $Name -Alias $Alias -DisplayName $DisplayName -PrimarySmtpAddress $smtp -MemberJoinRestriction $join -MemberDepartRestriction $depart
            Start-Sleep -s 10
            Set-DistributionGroup -Identity $alias -RequireSenderAuthenticationEnabled $RequireSenderAuthenticationEnabled
            }
        }
    ...
    if ($RecipientTypeDetails -eq "MailUniversalDistributionGroup")
        ...
        if ($ManagedBy)
            {
            New-DistributionGroup -Name $Name -Alias $Alias -DisplayName $DisplayName -PrimarySmtpAddress $smtp -MemberJoinRestriction $join -MemberDepartRestriction $depart -ManagedBy $ManagedBy
            Start-Sleep -s 10
            Set-DistributionGroup -Identity $alias -RequireSenderAuthenticationEnabled $RequireSenderAuthenticationEnabled
            }
            Else
